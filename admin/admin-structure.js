(() => {
  ...
  async function pageBuilder(){
    ...
    $$('[data-page-delete]').forEach(b=>b.onclick=async()=>{const p=data.find(x=>x.id===b.dataset.pageDelete);if(!confirm('"'+p.name+'" sayfasını silmek istiyor musun?'))return;const {error}=await A.state.db.from('esc_cms_pages').delete().eq('id',p.id);if(error)alert(error.message);else{toast('Sayfa silindi');pageBuilder()}});
  }
