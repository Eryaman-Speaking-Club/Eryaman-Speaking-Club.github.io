(function(){
  ...
  function mount(){
    ...
    const persist=()=>localStorage.setItem(STORAGE_KEY,JSON.stringify(source.data));
    ...
    function render(){
      ...
      source.data.forEach((item,index)=>{
        ...
        del.onclick=()=>{if(source.data.length<=1){alert('Keep at least one question.');return}source.data.splice(index,1);persist();render();refreshGame()};actions.append(edit,del);row.append(copy,actions);list.appendChild(row);
      });
    }
    saveBtn.onclick=()=>{if(!primary.value.trim())return;if((schema.kind==='choice'||schema.kind==='taboo')&&!secondary.value.trim())return;const item=makeItem(schema,category.value,primary.value,secondary.value);if(editing>=0)source.data.splice(editing,1,item);else source.data.push(item);persist();clearForm();render();refreshGame()};
